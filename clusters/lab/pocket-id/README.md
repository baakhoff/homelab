...
| Hermes | `HERMES_DASHBOARD_OIDC_*` in `clusters/lab/hermes/deployment.yaml`; id and secret in `oidc.sops.yaml` beside it | none: anyone the client admits has the whole dashboard. Restrict on the client's *Allowed User Groups* |
...
`?direct=1`, and for Hermes `https://hermes.lab.baakhoff.com/auth/callback`.
